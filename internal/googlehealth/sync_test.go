...
func TestClientReusesRefreshedTokenAcrossRequests(t *testing.T) {
	...
	client := NewClient(&http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		if req.Header.Get("Authorization") == "Bearer old-token" {
			oldTokenRequests++
			return jsonResponseStatus(http.StatusUnauthorized, `{"error":"expired"}`), nil
		}
	...
	if _, err := client.GetRaw(context.Background(), "old-token", "/first", retry); err != nil {
		t.Fatalf("first request: %v", err)
	}
	if _, err := client.GetRaw(context.Background(), "old-token", "/second", retry); err != nil {
